...
<script>
    module.exports = {
        ...
        methods: {
            ...
            set: function(data) {
                var vm = this;
                vm.pois.forEach(function(poi, key) {
                    poi.status = 'X';
                });
            ...
            getBlock: function(type) {
                ...
                vm.settings.blocks.forEach(function(block, key) {
                    if (block.id === type) {
            ...
            paint: function(type) {
                ...
                vm.pois.forEach(function(poi, key) {
                    ...
                    ctx.globalAlpha = 0.2;
                    ctx.fillRect(poi.x * vm.draw_ratio, poi.y * vm.draw_ratio, block.width * vm.draw_ratio, block.height * vm.draw_ratio);
                    ctx.globalAlpha = 1.0;
                    ...
                    if (poi.selected) {
                        ...
                        ctx.rect(poi.x * vm.draw_ratio, poi.y * vm.draw_ratio, block.width * vm.draw_ratio, block.height * vm.draw_ratio);
                        ctx.stroke();
                    }
                });
            }
        ...
        mounted: function() {
            ...
            vm.provider.canvas.addEventListener('click', function(evt) {
                var rect = vm.provider.canvas.getBoundingClientRect();
                var mouse = {
                ...
                vm.pois.forEach(function(poi, key) {
                    var block = vm.getBlock(poi.type);
                    if ((poi.x * vm.draw_ratio <= mouse.x && mouse.x <= (poi.x + block.width) * vm.draw_ratio) &&
                        (poi.y * vm.draw_ratio <= mouse.y && mouse.y <= (poi.y + block.height) * vm.draw_ratio)) {
                        poi.selected = true;;
                        vm.$emit('poi_selected', {
                            'name': poi.name
                        });
                    }
                });
